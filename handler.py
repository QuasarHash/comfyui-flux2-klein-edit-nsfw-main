...
def validate_input(job_input):
    ...
    if workflow is None:
        keys = list(job_input.keys()) if isinstance(job_input, dict) else []
        return (
            None,
            f"Missing 'workflow' parameter (got input keys: {keys}). "
            "Edit jobs need input.workflow + input.images — not prompt-only.",
        )
...
def _workflow_summary(workflow):
    """Compact workflow diagnostics for RunPod logs and API error responses."""
    save_nodes = []
    class_types = {}
    for node_id, node in workflow.items():
        class_type = node.get("class_type", "?")
        class_types[class_type] = class_types.get(class_type, 0) + 1
        if class_type in ("SaveImage", "SaveImageWithMetaData"):
            save_nodes.append(
                {
                    "id": str(node_id),
...
def handler(job):
    ...
    job_input = job["input"]
    job_id = job["id"]
    print(f"worker-comfyui - job={job_id} received")

    # Make sure that the input is valid
    validated_data, error_message = validate_input(job_input)
    if error_message:
        print(f"worker-comfyui - job={job_id} input validation failed: {error_message}")
    ...
    image_names = [img.get("name", "?") for img in input_images] if input_images else []
    print(
        f"worker-comfyui - job={job_id} workflow_summary={json.dumps(workflow_summary)} "
        f"images={image_names}"
    )

    if not workflow_summary["save_nodes"]:
        msg = "Workflow has no SaveImage node — nothing will be returned"
        print(f"worker-comfyui - job={job_id} ERROR: {msg}")
        return {"error": msg, "debug": {"workflow_summary": workflow_summary}}
    ...
    if not check_server(
        ...
        err = f"ComfyUI server ({COMFY_HOST}) not reachable after multiple retries."
        print(f"worker-comfyui - job={job_id} ERROR: {err} status={_comfy_server_status()}")
        return {
            "error": err,
            "debug": {"workflow_summary": workflow_summary, "comfy": _comfy_server_status()},
        }
    ...
    if input_images:
        ...
        if upload_result["status"] == "error":
            ...
            print(
                f"worker-comfyui - job={job_id} image upload failed: {upload_result['details']}"
            )
            return {
                "error": "Failed to upload one or more input images",
                "details": upload_result["details"],
                "debug": {"workflow_summary": workflow_summary, "images": image_names},
            }
    ...
    print(
        f"worker-comfyui - job={job_id} completed images={len(output_data)} "
        f"errors={len(errors)} workflow_nodes={workflow_summary['node_count']}"
    )
    if workflow_summary:
        final_result["debug"] = {"workflow_summary": workflow_summary}
    return final_result
